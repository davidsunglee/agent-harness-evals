...
`LocalShellBackend` does not inherit the parent process env unless configured. The adapter passes an explicit shell env built from a small allowlist (`HOME`, `LANG`, `LC_ALL`, `LC_CTYPE`, `TERM`, `TMPDIR`, `USER`, `LOGNAME`, `SHELL`, `TZ`, sanitized `PATH`) — *not* `os.environ.copy()` — with `<case-venv>/bin` forced to the front of `PATH`, `UV_PROJECT_ENVIRONMENT=<case-venv>`, `UV_NO_SYNC=1`, `PYTHONPATH=<repo>/src:<repo>`, and `PYTHONDONTWRITEBYTECODE=1`. Provider/API secrets the adapter process needs to talk to the model (`ANTHROPIC_API_KEY`, plus any other `*_API_KEY` / `*_TOKEN` / `AWS_*` values present in the parent shell) are deliberately not forwarded so a model-controlled `execute` command cannot exfiltrate them into traces or artifacts. The setup keeps agent-side `pytest`/`git` invocations on the harness-prepared case environment while preventing `uv run` from syncing into that shared venv or Python from adding `__pycache__` files to the submitted diff.
